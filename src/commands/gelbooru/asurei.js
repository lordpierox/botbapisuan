const Gelbooru  = require('gelbooru-api')
const { SlashCommandBuilder } = require('@discordjs/builders')
const { EmbedBuilder, Client } = require('discord.js')

module.exports = {

    data:new SlashCommandBuilder()
    .setName('asurei')
    .setDescription('Imagen casual asurei'),
    async execute(interaction, client)  {
        var linkId = "";
        let tags="asuka_langley_souryuu ayanami_rei   -1boy -2boys -multiple_boys -yaoi -futanari -model_building -loli -mass_production_eva -nude -rating:questionable  -rating:explicit -sexually_suggestive -3girls -4girls sort:random -ikari_shinji -solo";
        let date1;
        let linkImg = "";

        if(interaction.channel.nsfw){ 
            tags = "asuka_langley_souryuu ayanami_rei   rating:explicit -2boys  -multiple_boys -yaoi -futanari -model_building -loli -1boy -mass_production_eva sort:random -ikari_shinji -bdsm -violence -doll -newhalf ";
            //interaction.reply({content: "Este comando sólo se puede utilizar en los canales marcados como nsfw.", ephemeral:true}); 
            //return; 
        }
        
       GelbooruClient = new Gelbooru(tags);

        try {
            await GelbooruClient.getRandomPost(tags, 10, 0).then(post => { // get random post
                  linkId = (""+post.id); // print post id
                  linkImg = (""+post.file_url);
                  date1 = new Date(""+post.created_at);
                   
               }); 
    
            
           }catch(error) {
               console.log(error);
             }




             await interaction.deferReply()

             let imgasurei = new EmbedBuilder()
                   .setTitle("ASUREI")
                  .setColor("Random")
                  .setTimestamp(date1)
                  .setDescription("https://gelbooru.com/index.php?page=post&s=view&id="+linkId)
                  .setThumbnail("https://i.imgur.com/i21EvDR.gif")
                  .setImage(linkImg)
                  .setFooter({ text:"SukiBot"})
         
             interaction.followUp({ embeds: [imgasurei] })
         
             },
         }